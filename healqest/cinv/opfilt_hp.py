...
def calc_fini(alms, s_inv_filt):
    """Final operation turns the Wiener-filtered CMB cg-solution to the inverse-variance filtered CMB."""
...
class TFObj:
    ...
    def apply_tf(self, alms, maps, g, adjoint=False, g_tf=None):
        ...
        alms: array_like, optional
            Needed for forward operation, but can be omitted for adjoint operation. If provided in the latter
            case, it will be used as a buffer.
        maps: array_like
            Needed for adjoint operation, but can be omitted for forward operation. If provided in the latter
            case, it will be used as a buffer.
        g: geometry object
        """
        if adjoint:
...
class SkyInverseFilter:  # alm_filter_sinv_nocorr:
    """Inverse Signal filtering: S^-1 for T/EB or TEB without TE correlation."""
...
class SkyInverseFilterJoint(SkyInverseFilter):
    """Inverse signal filtering: S^-1 for TEB with TE correlation."""
