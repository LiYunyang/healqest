...
class qest(object):
    ...
    def __init__(self,config,qe,almbar1,almbar2,cltype='grad',u=None):
        ...
        self.q       = weights.weights(qe,max(self.lmax1,self.lmax2),self.config,cltype=self.cltype,u=self.u)
    ...
    def get_aresp(self,flX,flY,qe1=None,qe2=None):
        '''
    ...
    def get_source_estimator(self):
        ...
        q = weights.weights(est,max(self.lmax1,self.lmax2),self.config,cltype=self.cltype,u=self.u)
    ...
    def harden(self, flX, flY):
        ...
        ss = self.get_aresp(flX, flY, qe1='TTprf')
        es = self.get_aresp(flX, flY, qe1='TTprf', qe2=self.qe)
...
class qest_gmv(object):
    ...
    def __init__(self,config,qe,alm1all,alm2all,totalcls,cltype='grad',u=None):
        '''
    ...
    def eval(self):
        ...
        for i, est in enumerate(self.ests):
            ...
            q = weights.weights(est,max(self.lmax1,self.lmax2),self.config,cltype=self.cltype,u=self.u,totalcls=self.totalcls)
            glmsum = 0
            clmsum = 0
