...
def get_fl(config,mtype,use_unlCls=False):
    ...
    if use_unlCls:
        fl = 1.0/(config['cls']['ucmb'][sdict[mtype]][:lmax+1]+config['cls']['totres'][sdict[mtype]][:lmax+1])
    else:
        fl = 1.0/(config['cls']['lcmb'][sdict[mtype]][:lmax+1]+config['cls']['totres'][sdict[mtype]][:lmax+1])
    fl[:lmin] = 0
