...
def get_klm(dir_base,i,ktype='xx',qetype='TT',compname='all',fgseed=0,ilctype='cmbmv'):
    ...
    for qe in qes:
        ...
        if qe!='TT':
            # For non-TT parts, always use MV
            ilctype='cmbmv'

        # Choose response
        print('resp:',dir_base+'/lensrec_%s/respavg%s.npz'%(ilctype,qe))
        resp = np.load(dir_base+'/lensrec_%s/respavg%s.npz'%(ilctype,qe))['resp']
        resp[-100:]=np.inf
        ...
        if ktype=='xx':
            if i==0:
                ...
                mf = np.load(dir_base+'/lensrec_%s/plmstack%s_%s.alm.npz'%(ilctype,qe,ktype))
                y  = np.load(dir_base+'/lensrec_%s_cmbNG_fgG_partialNG/fgseed%d/%s/plm%s_%s_%s_%s_x_%s_fgseed%d.alm.npz'%(ilctype,fgseed,qe,qe,ii,jj,fid,fid,fgseed))['glm']
                k  = hp.almxfl(y-(mf['mf'])/(mf['nsim']),0.5*l*(l+1)*1/resp)
            else:
                # all Gaussian foreground
                mf = np.load(dir_base+'/lensrec_%s/plmstack%s_%s.alm.npz'%(ilctype,qe,ktype) )
                y  = np.load(dir_base+'/lensrec_%s/plm%s_%s_%s_%s_x_%s.alm.npz'%(ilctype,qe,ii,jj,fidG,fidG))['glm']
            ...
            if i>0:
                mf = np.load(dir_base+'/lensrec_%s_cmbNG_fgG_partialNG/fgseed%d/%s/plmstack%s_%s_%s_x_%s_fgseed%d.alm.npz'%(ilctype,fgseed,qe,qe,ktype,fidG,fid,fgseed))
                y  = np.load(dir_base+'/lensrec_%s_cmbNG_fgG_partialNG/fgseed%d/%s/plm%s_%s_%s_%s_x_%s_fgseed%d.alm.npz'%(ilctype,fgseed,qe,qe,ii,jj,fidG,fid,fgseed))['glm']
                k  = hp.almxfl(y-(mf['mf']-y)/(mf['nsim']-1.0),0.5*l*(l+1)*1/resp)
        elif ktype=='0x': # by definition i>0
            if i>0:
                mf = np.load(dir_base+'/lensrec_%s_cmbNG_fgG_partialNG/fgseed%d/%s/plmstack%s_%s_%s_x_%s_fgseed%d.alm.npz'%(ilctype,fgseed,qe,qe,ktype,fid,fidG,fgseed))
                y  = np.load(dir_base+'/lensrec_%s_cmbNG_fgG_partialNG/fgseed%d/%s/plm%s_%s_%s_%s_x_%s_fgseed%d.alm.npz'%(ilctype,fgseed,qe,qe,ii,jj,fid,fidG,fgseed))['glm']
                k  = hp.almxfl(y-(mf['mf']-y)/(mf['nsim']-1.0),0.5*l*(l+1)*1/resp)
...
dir_p    = '/tmp/'
dir_cls  = dir_base+'/clkk_%s/%s/%s/'%(ilctype,qe,comp)
...
if fgseed==0:
    if i==0:
        kxx = get_klm(dir_base,i,ktype='xx',qetype=qe,compname=comp,fgseed=fgseed,ilctype=ilctype); kmapxx = hp.alm2map(kxx,2048); hp.write_map(dir_p+'/kmapxx_%d.fits'%i,kmapxx,overwrite=True,dtype=np.float32)
        spice(qid,dir_cls,file_mask,dir_p,i,ktype='xxxx',comp=comp,fgseed=fgseed)
        os.remove(dir_p+'/kmapxx_%d.fits'%i)
    else:   
        kxx = get_klm(dir_base,i,ktype='xx',qetype=qe,compname=comp,fgseed=fgseed,ilctype=ilctype); kmapxx = hp.alm2map(kxx,2048); hp.write_map(dir_p+'/kmapxx_%d.fits'%i,kmapxx,overwrite=True,dtype=np.float32)
        kxy = get_klm(dir_base,i,ktype='xy',qetype=qe,compname=comp,fgseed=fgseed,ilctype=ilctype); kmapxy = hp.alm2map(kxy,2048); hp.write_map(dir_p+'/kmapxy_%d.fits'%i,kmapxy,overwrite=True,dtype=np.float32)
        kyx = get_klm(dir_base,i,ktype='yx',qetype=qe,compname=comp,fgseed=fgseed,ilctype=ilctype); kmapyx = hp.alm2map(kyx,2048); hp.write_map(dir_p+'/kmapyx_%d.fits'%i,kmapyx,overwrite=True,dtype=np.float32)
        spice(qid,dir_cls,file_mask,dir_p,i,ktype='xxxx',comp=comp,fgseed=fgseed)
    ...
    if i==0:
        ...
        kx0 = get_klm(dir_base,i,ktype='x0',qetype=qe,fgseed=fgseed,ilctype=ilctype); kmapx0 = hp.alm2map(kx0,2048) ; hp.write_map(dir_p+'/kmapx0_%d.fits'%i,kmapx0,overwrite=True,dtype=np.float32)
        k0x = get_klm(dir_base,i,ktype='0x',qetype=qe,fgseed=fgseed,ilctype=ilctype); kmap0x = hp.alm2map(k0x,2048) ; hp.write_map(dir_p+'/kmap0x_%d.fits'%i,kmap0x,overwrite=True,dtype=np.float32)
